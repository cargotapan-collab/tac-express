...
export function KpiTile({
  ...
  return (
    <article
      className={cn(
        "flex flex-col gap-3 rounded border border-chart-grid bg-card p-4",
...
function DeltaChip({ label, trend }: { label: string; trend: Trend }) {
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 rounded border border-chart-grid",
...
function buildStepPath(points: SparkPoint[]): { d: string; fillD: string } {
  ...
  const coords = points.map((p, i) => {
    const x = i * stepX;
    const y = 22 - ((p.y - min) / range) * 20; // pad 2px top/bottom
    return { x, y };
  });
  ...
  for (let i = 1; i < coords.length; i++) {
    ...
    const cur = coords[i]!;
    // stepAfter: hold previous y, then jump
