...
/**
 * Stacked horizontal segment bar — replaces composition donuts.
 * At small N this still communicates proportions clearly; at large N it
 * doesn't hide segments behind one another.
 *
 * Server-safe.
 */
    ...
    <figure className={cn("flex flex-col gap-3", className)}>
      <div
        ...
        aria-label={`Composition: ${enriched
          ...
          .join(", ")}`}
        className="flex h-6 w-full overflow-hidden rounded border border-chart-grid"
      >
        {enriched.map((seg) => (
          <div
            key={seg.key}
