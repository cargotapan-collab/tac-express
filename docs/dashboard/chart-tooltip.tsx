...
interface OrbitalTooltipProps extends TooltipProps<number, string> {
 ...
 * `<Tooltip content={<OrbitalTooltip />} />` or shadcn's
 * `<ChartTooltip content={<OrbitalTooltip />} />`.
 *
...
export function OrbitalTooltip({
  ...
  return (
    <div
      role="tooltip"
      className={cn(
        "min-w-32 rounded border border-chart-grid bg-popover text-popover-foreground",
        "p-2",
      )}
    >
      {label !== undefined ? (
