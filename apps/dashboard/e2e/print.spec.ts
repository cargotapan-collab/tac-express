...
test.describe("Print routes — visual baselines (issue #17)", () => {
  // Local runs skip cleanly when nothing is configured; CI must hard-fail
  // on missing auth so a misconfigured runner doesn't silently bank a
  // green check it never actually ran.
  test.skip(
    !IS_CI && !HAS_AUTH_SESSION,
    "Local run — no auth session. Set E2E_USER_EMAIL + E2E_USER_PASSWORD to enable.",
  )

  test.beforeAll(() => {
    if (IS_CI && !HAS_AUTH_SESSION) {
      throw new Error(
        "[print.spec] Running in CI without an auth session — set the E2E_USER_EMAIL / E2E_USER_PASSWORD repo secrets.",
      )
    }
    // Per-fixture env vars stay soft (per-test skips remain) so coverage
    // can roll out incrementally per the issue spec — but emit a warn so
    // a CI run that passes with zero fixtures is visibly empty.
    if (IS_CI) {
      const missing = [
        !INVOICE_ID && "E2E_INVOICE_ID",
        !AWB && "E2E_AWB",
        !MANIFEST_ID && "E2E_MANIFEST_ID",
      ].filter(Boolean)
      if (missing.length > 0) {
        console.warn(
          `[print.spec] Print-baseline fixtures not set: ${missing.join(", ")}. Affected tests will skip.`,
        )
      }
    }
  })
  ...
  test("invoice print view", async ({ page }) => {
    ...
    // Print target must actually mount before snapshotting; silent
    // .catch() previously masked a missing target as a successful
    // baseline of a loading/error state.
    await expect(page.locator("[data-print-target]")).toBeVisible({ timeout: 5000 })
