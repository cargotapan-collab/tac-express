    ...
    <div className="space-y-6">
      ...
      {!query.isLoading && filtered.length > 0 && (
        <div className="border border-border bg-background">
          <div className="grid grid-cols-[150px_110px_140px_1fr_140px_140px_32px] border-b border-border bg-muted/40 px-3 py-2 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
            <span>Time</span>
            <span>Action</span>
            <span>Entity</span>
...
function Detail({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <p className="break-all font-mono text-[11px]">{value}</p>
    </div>
  )
}
