  ...
  return (
    <div className="bg-background p-4">
      ...
      {stats && stats.length > 0 && (
        <ul className="mt-2 flex flex-wrap items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
          {stats.map((s) => (
            <li key={s.label}>
              {s.label}{" "}
...
function Empty({ children }: { children: React.ReactNode }) {
  return (
    <p className="border border-dashed border-border bg-muted/20 px-3 py-3 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
      {children}
    </p>
  )
}
