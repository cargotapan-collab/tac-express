  ...
  if (changed.length === 0) {
    return (
      <p
        ...
        className={cn(
          "border border-dashed border-border px-3 py-2 font-mono text-[10px] uppercase tracking-widest text-muted-foreground",
          className
        )}
      >
    ...
    >
      ...
      <ul className="divide-y divide-border/60">
        {changed.map((row) => (
          ...
          >
            <span className="truncate font-mono text-[11px] font-semibold tracking-widest">
              {row.key}
            </span>
...
function DiffCell({
  ...
  return (
    <pre
      className={cn(
        "max-h-32 overflow-auto whitespace-pre-wrap break-all border border-transparent px-2 py-1 font-mono text-[11px] leading-snug",
        isHighlighted &&
          side === "before" &&
